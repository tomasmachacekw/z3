...
spacer_matrix::spacer_matrix(unsigned m, unsigned n)
    ...
    for (unsigned i = 0; i < m; ++i) {
        vector<rational> v;
        for (unsigned j = 0; j < n; ++j) { v.push_back(rational(0)); }
...
void spacer_matrix::print_matrix() {
    ...
    for (const auto &row : m_matrix) {
        for (const auto &element : row) { verbose_stream() << element << ", "; }
        verbose_stream() << "\n";
    }
    verbose_stream() << "\n";
}
...
bool spacer_matrix::is_lin_reltd(unsigned i, unsigned j, rational& coeff1,
    ...
    for(unsigned k = 0; k < m_num_rows; k++) {
        if( ((coeff1 * m_matrix[k][i]) + (coeff2 * m_matrix[k][j]) + off) != rational::zero()) {
            TRACE("cvx_dbg_verb", tout << "Didn't work for " << m_matrix[k][i] << " and " << m_matrix[k][j] << " with coefficients " << coeff1 << " , " << coeff2 << " and offset " << off << "\n";);
            return false;
        }
    }
...
bool spacer_matrix::compute_linear_deps(spacer_matrix &eq)
    const {
    SASSERT(m_num_rows > 1);
    ...
    for(unsigned i = 0; i < m_num_cols; i++) {
        for(unsigned j = i + 1; j < m_num_cols; j++) {
            if (is_lin_reltd(i, j, coeff1, coeff2, off)) {
                ...
                TRACE("cvx_dbg_verb", tout << "Adding row "; for (rational r : lin_dep) tout << r << " "; tout << "\n";);
                //reset everything
