...
bool lemma_cluster_finder::are_neighbours(expr_ref antiU_result,
    ...
    for (unsigned j = 0, sz = s1.get_num_bindings(); j < sz; j++) {
        ...
        if (is_uninterp_const(r.get_expr()) &&
            ...
            // HG: right now, we are concerned only with difference in
            // interpreted constants Not sure whether we need this for now.
    ...
    for (unsigned j = 0, sz = uninterp_s1.size(); j < sz; j++) {
        if (!m.are_equal(uninterp_s1.get(j), uninterp_s2.get(j))) {
            ...
            // If both of them are the same uninterpreted constant, they
            // wouldn't be antiunified.
            return false;
        }
    }
    return true;
...
bool lemma_cluster_finder::are_neighbours(const expr_ref &cube,
                                          const expr_ref &lcube, expr_ref &pat,
                                          substitution &sub1,
                                          substitution &sub2) {
    anti_unifier anti(m);
...
void lemma_cluster_finder::operator()(lemma_ref &lemma) {
    ...
    CTRACE("cluster_stats",
           !neighbours.empty() && num_vars_in_pattern > 0 &&
               !is_general_pattern,
           tout << "Failed to find a general pattern for cluster. Lemma is: "
                << lemma->get_cube() << " patterns are " << patterns << "\n";);
