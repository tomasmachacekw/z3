...
// however, the algorithm is incomplete: it returns such a res iff the res \in {antiU(cube, e) | e \in fmls}
bool lemma_cluster_finder::anti_unify_n_intrp(expr_ref &cube,
    ...
    TRACE("cluster_stats_verb", tout << "Trying to generate a general pattern for " << cube <<
          " neighbours are " << fmls << "\n";);
    ...
    for (expr *e : patterns) {
        ...
        TRACE("cluster_stats_verb", tout << "Checking pattern " << mk_pp(e, m) << "\n";);
