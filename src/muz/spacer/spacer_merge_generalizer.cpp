...
void lemma_merge_generalizer::to_real(expr_ref &fml) {
    ...
    if(m_arith.is_arith_expr(fml)) {
        ...
        fml = m.mk_app(fml_app->get_family_id(), fml_app->get_decl_kind(), nw_args.size(), nw_args.c_ptr());
    }
}
...
rational lemma_merge_generalizer::get_lcm(expr *e) {
    compute_lcm g(m);
    for_each_expr(g, e);
    TRACE("merge_dbg_verb", tout << "lcm of " << mk_pp(e, m) << " is " << g.m_val << "\n";);
    return g.m_val;
}
...
void lemma_merge_generalizer::mul_and_simp(expr_ref &fml, rational num) {
    ...
    SASSERT(!num.is_zero());
    SASSERT(num.is_pos());
    TRACE("merge_dbg_verb", tout <<"mul and simp called with " << mk_pp(fml, m) << "\n";);
    ...
    for(unsigned i = 0; i < N; i++) {
        ...
        if(m_arith.is_mul(chld)) {
            ...
            nw_args.push_back(m_arith.mk_mul(numeral, to_app(chld)->get_arg(1)));
        }
        else {
...
void lemma_merge_generalizer::to_int(expr_ref &fml) {
    ...
    TRACE("merge_dbg_verb", tout << "to int finished " << mk_pp(fml, m) << "\n";);
}
...
void lemma_merge_generalizer::normalize(expr_ref & fml) {
    ...
    for(expr* e : fml_vec) {
        if(!(m_arith.is_arith_expr(e) || m.is_eq(e)))
            continue;
        ...
        if(lcm != 1) {
            ...
            TRACE("merge_dbg_verb", tout << "mul and simp reduced lhs to " << mk_pp(lhs, m) << " and rhs to " << mk_pp(rhs, m) << "\n" ;);
        }
        to_int(lhs);
        to_int(rhs);
        app *norm_e = m.mk_app(e_app->get_family_id(), e_app->get_decl_kind(), lhs, rhs);
        rw_fml.push_back(to_expr(norm_e));
    }
    fml = mk_and(rw_fml);
}
void lemma_merge_generalizer::to_real(const expr_ref_vector& fml, expr_ref& nw_fml) {
    expr_ref lhs(m), rhs(m);
    expr_ref_vector rw_fml(m);
    for(auto& e : fml) {
        if( !(m.is_eq(e) || m_arith.is_arith_expr(e)))
            continue;
        app* e_app = to_app(e);
        ...
        to_real(rhs);
        to_real(lhs);
        rw_fml.push_back(expr_ref(to_expr(m.mk_app(e_app->get_family_id()
                                                   , e_app->get_decl_kind(), lhs, rhs)), m));
    }
...
bool lemma_merge_generalizer::core(lemma_ref &lemma) {
    ...
    CTRACE("merge_dbg", !exact, tout << "Convex closure introduced new variables. Closure is" << mk_and(cls) << "\n";);
    if(!exact) {
...
void lemma_merge_generalizer::rewrite_pattern(expr *pattern,
    ...
    TRACE("merge_dbg_verb", tout << "Rewrote all vars into u_consts " << mk_pp(pattern, m) << " into "
                            << rw_pattern << "\n";);
    bool all_ints = true;
    ...
    if(!all_ints) {
        ...
        TRACE("merge_dbg_verb", tout << "To real produced "
                                << rw_pattern << "\n";);
        for (unsigned i = 0; i < m_dim_vars.size(); i++) {
...
bool wide_generalizer::should_apply(const expr* lit, rational val, rational n) {
    ...
    expr* neg_lit;
    if(m.is_not(lit, neg_lit)) {
        return !should_apply(neg_lit, val, n);
    }
...
void wide_generalizer::operator()(lemma_ref & lemma) {
    ...
    lemma_cluster *lc = pt.get_cluster(lemma);
    if(lc == nullptr) return;
    const expr_ref& pattern = lc->get_pattern();
    expr_ref lit(m);
    if(!mono_var_pattern(pattern, lit)) return;
    TRACE("merge_dbg", tout << "Applying widening on " << mk_pp(lemma->get_expr(), m) << " literal is " << lit << "\n";);
    ...
    for(rational n : m_consts){
        if(should_apply(lit, val, n)) {
            ...
            if(is_ind) {
                ...
                lemma->set_level(uses_level);
                val = n;
                TRACE("merge_dbg", tout << "widening succeeded with " << n << "\n";);
            }
