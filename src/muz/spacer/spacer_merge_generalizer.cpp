...
lemma_merge_generalizer::lemma_merge_generalizer(context &ctx)
  : lemma_generalizer(ctx), m(ctx.get_ast_manager()), m_arith(m), m_use_sage(ctx.use_sage()){
  if(m_use_sage)
    m_kernel = new Sage_kernel(0, 0);
}
/* Guards! Guards! */
...
/*
  ...
  (>= (+ (* k_1 t_1) (* k_2 t_2)) k_3) with all k >= 0
   ...
   (>= (+ t_1 t_2) 0)
*/
bool lemma_merge_generalizer::half_plane_03(const expr_ref &literal,
                                            const expr *pattern,
...
*/
bool lemma_merge_generalizer::half_plane_XX(const expr_ref &literal,
    ...
    if (gt_or_geq(fst) && lt_or_leq(snd)) {
        ...
        conj =
            m.mk_app(to_app(snd)->get_family_id(), to_app(snd)->get_decl_kind(),
                     m_arith.mk_add(t1, t2), m_arith.mk_numeral(k1 + k2, true));
        conjectures.push_back(conj);
        return true;
    } else if (m.is_eq(snd)) {
        // from t2 = k2 & t1 op k1 conjecture t1 + t2 op k1 + k2
        conj =
            m.mk_app(to_app(fst)->get_family_id(), to_app(fst)->get_decl_kind(),
                     m_arith.mk_add(t1, t2), m_arith.mk_numeral(k1 + k2, true));
        conjectures.push_back(conj);
        return true;
    }
...
bool lemma_merge_generalizer::core(lemma_ref &lemma) {
    ...
    m_cvx_cls.reset(lc->get_size(), n_vars);
    ...
    if (half_plane_03(normalizedCube, mk_and(non_bool_lit_pattern),
                      conjuncts)) {
        TRACE("merge_strategies",
    ...
    if (half_plane_XX(normalizedCube, mk_and(non_bool_lit_pattern),
                      conjuncts)) {
        TRACE("merge_strategies",
        ...
        } else if (check_inductive_and_update(lemma, conjuncts,
                                              non_var_or_bool_Literals)) {
            m_st.half_planeXX_success++;
            IF_VERBOSE(1, verbose_stream() << "MXX ");
            return true;
...
bool lemma_merge_generalizer::check_inductive_and_update_multiple(
    lemma_ref &lemma, expr_ref_vector &conjs,
    expr_ref_vector &non_var_or_bool_Literals) {
    ...
    for (auto *conj : conjs) {
        ...
        if (check_inductive_and_update(lemma, conj_and,
                                       non_var_or_bool_Literals))
            return true;
    }
    return false;
...
void lemma_merge_generalizer::collect_statistics(statistics &st) const {
    ...
    st.update("SPACER merge half plane prog success",
