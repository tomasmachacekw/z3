...
#include "muz/spacer/spacer_legacy_frames.h"
    ...
    ptr_vector<func_decl>
        m_predicates; // temp vector used with find_predecessors()
    stats m_stats;
  ...
  public:
    ...
    void get_lemmas_at_frame_geq(unsigned level, lemma_ref_vector &out) const {
        ...
        for (auto &lemma : temp) {
            if (lemma->level() >= level) { out.push_back(lemma); }
        }
    }
    ...
    void add_pattern(expr_ref p) { m_seen_lemma_patterns.push_back(p); }
    expr_ref_vector seen_patterns() { return m_seen_lemma_patterns; }
