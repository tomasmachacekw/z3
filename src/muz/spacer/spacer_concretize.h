...
namespace spacer {
class concretize {
    using expr_expr_map = obj_ref_map<ast_manager, expr, expr *>;
    using expr_rat_map = obj_ref_map<ast_manager, expr, rational>;
    ...
    ///     forall x in uninterp_consts(term) :: ((lb[x] <= x <= ub[x]) ==>
    ///     term)
    ...
    // Check whether \p term is to be treated as a separate partition according
    // to \p pattern
